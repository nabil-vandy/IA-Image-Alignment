...
# --- STEP 2: LIVE ALIGNMENT PHASE ---
    ...
    # 2. RENDER BUTTON IMMEDIATELY AFTER (Below the video controls)
    # We use type="primary" which our CSS above turns GREEN.
    if st.button("📸 TAP HERE TO CAPTURE", type="primary", use_container_width=True):
        if ctx.state.playing and ctx.video_processor and ctx.video_processor.clean_frame is not None:
            ...
            st.warning("⚠️ Waiting for camera stream... Press 'Start' first.")

# --- STEP 3: RESULT PHASE ---
else:
    st.header("Step 3: Results")
    
    # This button will also be Green due to our CSS, which is fine for a "Go" action
